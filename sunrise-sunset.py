...
def openCurtains(scheduler):
    for x in open_curtains:
        telnetSet(x,"1")
        print(x)
    scheduler.add_job(openCurtains, 'date', run_date=sunTime("sunrise",date.today() + timedelta(days = 1)), args=[scheduler] )
    logging.debug(scheduler.print_jobs())


if __name__ == '__main__':
    scheduler = BackgroundScheduler()
    scheduler.add_job(closeCurtains, 'date', run_date=datetime.now(), args=[scheduler] )
